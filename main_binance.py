...
class WhaleEngine:
    ...
    def fetch_binance_public(self, symbol, interval="5m", limit=120):
        """Binance public REST API — no keys needed, real candle data!"""
        try:
            ...
            log.warning(f"Binance public {resp.status_code} for {symbol}")
            return None
        except Exception as e:
    ...
    def fetch_data(self, symbol, limit=120, vol_map=None):
        # Binance public API — REAL data, no keys needed!
        for attempt in range(3):
            result_5m  = self.fetch_binance_public(symbol, "5m",  limit)
            result_15m = self.fetch_binance_public(symbol, "15m", 60)
            if result_5m is not None and result_15m is not None:
                opens, highs, lows, closes, vols = result_5m
                closes15 = result_15m[3]
                log.info(f"Binance Real: {symbol} @ ${round(float(closes[-1]),4)}")
                return opens, highs, lows, closes, vols, closes15
            log.warning(f"Binance retry {attempt+1}/3 for {symbol}")
            time.sleep(2)
        log.error(f"Binance failed after 3 retries: {symbol} — skipping")
        return None
