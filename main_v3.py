...
def market_regime(closes, highs, lows):
    ...
    ema200 = ema_calc(closes, min(200, len(closes)-1))[-1]
...
def darvas_box_strategy(opens, highs, lows, closes, vols, box_period=20, vol_period=20):
    ...
    # Step 2: Volume confirmation — current vol > 20-period MA
    vol_ma = float(np.mean(vols[-vol_period:]))
    volume_confirmed = current_volume > vol_ma

    # Step 3: Breakout detection
    # Darvas: Price must CLOSE above resistance with volume surge
    breakout_up   = current_close > resistance and volume_confirmed
    breakout_down = current_close < support    and volume_confirmed
...
class WhaleEngine:
    ...
    def score_signal(self, opens, highs, lows, closes, vols, closes15):
        ...
        # Adaptive MIN_SCORE based on regime + volatility
        if regime == "SIDEWAYS":
            adaptive_min = 8   # SIDEWAYS = very strict, avoid random trades
        elif atr_pct > 1.5:
            adaptive_min = 5   # High volatility trending = more opportunities
        elif atr_pct < 0.5:
            adaptive_min = 7   # Low volatility = strict filter
        else:
            adaptive_min = 6   # Normal trending market
        ...
        if direction:
            if regime == "BULL" and direction == "buy":
                ...
                score = max(0, score - 2)  # Penalize — avoid sideways trades
                reasons.append("SIDEWAYS-AVOID")
            else:
                # Counter-trend = penalize score
                score = max(0, score - 2)
        ...
        # 6. Volume confirmation (+2) — real volume spike
