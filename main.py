...
TP_MULT     = 2.0      # Bigger TP — more profit per trade
...
class WhaleEngine:
    ...
    def score_signal(self, opens, highs, lows, closes, vols, closes15):
        ...
        # 4. Regime filter — bonus only, no blocking
        if direction:
            if regime == "BULL" and direction == "buy":
                ...
                # No penalty — RSI/BB valid in sideways
            else:
                reasons.append("COUNTER-TREND")
                # No block — let indicators decide
            ...
            try:
                ...
                if direction and score >= adaptive_min and self.can_trade(score):
                    ...
                    if direction == "buy":
                        ...
                        # SL = Recent Swing Low (Price Action based!)
                        swing_low = round(float(np.min(lows[-10:])), 6)
                        sl = round(swing_low - (av * 0.05), 6)  # tiny buffer below swing low
                    else:
                        tp = round(price - av * TP_MULT, 6)
                        # SL = Recent Swing High (Price Action based!)
                        swing_high = round(float(np.max(highs[-10:])), 6)
                        sl = round(swing_high + (av * 0.05), 6)  # tiny buffer above swing high
