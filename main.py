...
class WhaleEngine:
    ...
    def fetch_bybit_public(self, symbol, interval="5", limit=120):
        """Bybit public REST API — globally accessible, real candle data!"""
        try:
            # Bybit interval format: "5" for 5min, "15" for 15min
            ...
            log.warning(f"Bybit public {resp.status_code} for {symbol}")
            return None
        except Exception as e:
            log.warning(f"Bybit public failed {symbol}: {e}")
            return None

    def fetch_data(self, symbol, limit=120, vol_map=None):
        # Bybit public API — REAL data, globally accessible, no keys needed!
        for attempt in range(3):
            result_5m  = self.fetch_bybit_public(symbol, "5",  limit)
            result_15m = self.fetch_bybit_public(symbol, "15", 60)
            if result_5m is not None and result_15m is not None:
                opens, highs, lows, closes, vols = result_5m
                closes15 = result_15m[3]
                log.info(f"Bybit Real: {symbol} @ ${round(float(closes[-1]),4)}")
                return opens, highs, lows, closes, vols, closes15
            log.warning(f"Bybit retry {attempt+1}/3 for {symbol}")
            time.sleep(2)
        log.error(f"Bybit failed after 3 retries: {symbol} — skipping")
        return None
