...
MARGIN_BLAST     = 500.0   # BB Blast = 50% of $1000 capital
LEVERAGE         = 10
HISTORY_FILE     = "history.json"

# ── Scalping Config ─────────────────────────
MIN_SCORE   = 2        # Very low — BB signals are enough alone
VOL_MULT    = 1.3      # OKX volume spike
# BB Settings — research proven for 5-min scalping
BB_PERIOD   = 20       # Standard
BB_STD      = 2.0      # Standard 2.0
BB_FAST_P   = 10       # Fast BB for scalping confirmation
BB_FAST_S   = 1.5      # Fast BB std
# S/R Consolidation
SR_PERIOD   = 20       # Period to find swing highs/lows
SR_THRESH   = 0.001    # 0.1% proximity to S/R level
ATR_PERIOD  = 10

# TP/SL — BB mid as TP target (research-backed)
TP_MULT     = 1.5      # Normal trade TP
SL_MULT     = 1.0      # Normal trade SL
TRAIL_STEP  = 0.15     # Tight trail — exit near BB mid
...
class WhaleEngine:
    ...
    def score_signal(self, opens, highs, lows, closes, vols, closes15):
        ...
        r      = 50.0  # dummy RSI for compatibility
        ...
        squeeze = band_width_now < avg_band_width * 0.95  # bands slightly narrower than avg

        # Big candle = blast signal
        big_candle = abs(body_now) > avg_body * 1.5
        ...
        # ════════════════════════════════════════
        # STRATEGY 5: BB MIDLINE CROSS (Most Frequent!)
        # Research: Price crossing middle band = momentum shift
        # Very frequent signal — good for high frequency scalping
        # ════════════════════════════════════════
        if direction is None and len(closes) >= 3:
            prev_close = float(closes[-2])
            # Price crossed above midline — bullish
            if prev_close < mid and price > mid:
                score += 4
                direction = "buy"
                reasons.append("BB MID CROSS UP")
            # Price crossed below midline — bearish
            elif prev_close > mid and price < mid:
                score += 4
                direction = "sell"
                reasons.append("BB MID CROSS DN")
