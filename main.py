...
footer { text-align: center; color: var(--muted); font-size: 10px; padding: 16px; }"""
        ...
        html = (
            ...
            "var pairs={'btcusdt':'BTC-USDT-SWAP','ethusdt':'ETH-USDT-SWAP','solusdt':'SOL-USDT-SWAP','bnbusdt':'BNB-USDT-SWAP','xrpusdt':'XRP-USDT-SWAP','dogeusdt':'DOGE-USDT-SWAP'};"
            "var prevPrices={};"
            "function fetchPrices(){"
            "  var keys=Object.keys(pairs);"
            "  keys.forEach(function(sym){"
            "    var instId=pairs[sym];"
            "    fetch('https://www.okx.com/api/v5/market/ticker?instId='+instId)"
            "    .then(function(r){return r.json();})"
            "    .then(function(j){"
            "      if(j.code!='0'||!j.data||!j.data[0])return;"
            "      var d=j.data[0];"
            "      var price=parseFloat(d.last);"
            "      var open24=parseFloat(d.open24h);"
            "      var chg=((price-open24)/open24)*100;"
            ...
            "      pe.style.color=price>prev?'var(--green)':price<prev?'var(--red)':'var(--text)';"
            "      pe.style.transition='color 0.3s';"
