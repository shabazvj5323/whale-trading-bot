# main.py – WhaleTrader Ultimate (GitHub 15-min | OKX-first fetch)
import time
import logging
import os
import json
...
LIQ_SWEEP_CONFIRM_BARS = 1
ENABLE_OI_FUNDING = False   # Keep False to avoid extra API calls
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            if data.get("code") == "0" and data.get("data"):
                raw = data["data"][::-1]  # reverse so newest last
                arr = np.array([[float(c[1]), float(c[2]), float(c[3]),
                                 float(c[4]), float(c[5])] for c in raw])
                return arr[:,0], arr[:,1], arr[:,2], arr[:,3], arr[:,4]
        log.warning(f"OKX public returned {resp.status_code} for {symbol} {interval}")
        return None
...
def fetch_candles(symbol, interval="5m", limit=120):
    # Try OKX first (more reliable on GitHub Actions)
...
# =====================================================================
# ENGINE (full – includes build_dashboard)
# =====================================================================
class WhaleEngine:
    def __init__(self):
        self.dashboard = []
    ...
    # ---- SCORING (Liquidation Hunting integrated) ----
    def score_signal(self, data, symbol):
        ...
        # 9b. Multiple Liquidation Levels
    ...
    # ---- DASHBOARD (FULL – reused from previous version) ----
    def build_dashboard(self):
        ...
        # ---- STRATEGY TRACKER (forever cumulative) ----
        STRATEGY_NAMES = [
