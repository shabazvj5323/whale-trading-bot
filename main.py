...
class WhaleEngine:
    ...
    def score_signal(self, opens, highs, lows, closes, vols, closes15):
        ...
        # 4. Regime filter — BULL=BUY only, BEAR=SELL only
        if direction:
            if regime == "BULL":
                if direction == "buy":
                    score += 2
                    ...
                    direction = None   # BULL mein SELL = completely blocked!
                    score = 0
                    reasons.append("BULL-NO-SELL")
            elif regime == "BEAR":
                if direction == "sell":
                    score += 2
                    reasons.append("BEAR TREND")
                else:
                    direction = None   # BEAR mein BUY = completely blocked!
                    score = 0
                    reasons.append("BEAR-NO-BUY")
            elif regime == "SIDEWAYS":
                score = max(0, score - 2)
                reasons.append("SIDEWAYS-AVOID")
    ...
    def build_dashboard(self):
        ...
        for d in self.dashboard:
            ...
            monitor_rows += (
                ...
                "<td class='entry-cell'>" + entry_val + "</td>"
        ...
        css = (
            ...
            ".entry-cell{color:var(--amber);font-family:var(--mono);font-size:12px;}.tp-cell{color:var(--green);font-family:var(--mono);font-size:12px;}"
        ...
        html = (
            ...
            "<div class='tbl-card'><div class='tbl-wrap'><table><thead><tr><th>Pair</th><th>Live Price</th><th>24h</th><th>Signal</th><th>Score</th><th>Indicators</th><th>Entry Price</th><th>Take Profit</th><th>Stop Loss</th></tr></thead><tbody>" + monitor_rows + "</tbody></table></div></div>"
