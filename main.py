# main.py – WhaleTrader Ultimate (GitHub 15-min, CCXT multi-exchange)
import time
import logging
import os
import json
...
# =====================================================================
# DATA FETCHING – USING CCXT (Multi-Exchange)
# =====================================================================
# Create exchange instances (no API keys needed for public OHLCV)
exchanges = {
    'okx': ccxt.okx({
    ...
    # Binance is blocked on GitHub, so we don't include it
}

def fetch_candles_ccxt(symbol, interval="5m", limit=120):
    """Fetch candles from multiple exchanges using ccxt."""
    # Convert interval to ccxt format (e.g., '5m' -> '5m')
    for name, exchange in exchanges.items():
        try:
            # For OKX swap, symbol might need to be converted (e.g., BTC/USDT -> BTC/USDT:USDT)
            if name == 'okx':
                # OKX swap symbol format: BTC/USDT:USDT
                market_id = symbol.replace('/', '/') + ':USDT'
            else:
                market_id = symbol
            # Fetch
            ohlcv = exchange.fetch_ohlcv(market_id, interval, limit=limit)
            if ohlcv and len(ohlcv) > 0:
                ...
                closes = arr[:, 4]
                volumes = arr[:, 5]
                log.info(f"Fetched {len(ohlcv)} candles from {name} for {symbol} {interval}")
                return opens, highs, lows, closes, volumes
        except Exception as e:
            log.warning(f"CCXT {name} failed for {symbol} {interval}: {str(e)[:100]}")
            continue
    # If all fail, return None
    log.error(f"All exchanges failed for {symbol} {interval}")
    return None

def fetch_symbol_data_enhanced(symbol):
    """Fetch 1h, 15m, 5m, 1m data using ccxt."""
    with ThreadPoolExecutor(max_workers=4) as executor:
        f1h = executor.submit(fetch_candles_ccxt, symbol, "1h", 100)
        f15m = executor.submit(fetch_candles_ccxt, symbol, "15m", 120)
        f5m = executor.submit(fetch_candles_ccxt, symbol, "5m", 120)
        f1m = executor.submit(fetch_candles_ccxt, symbol, "1m", 60)
        r1h = f1h.result()
        r15m = f15m.result()
        r5m = f5m.result()
        r1m = f1m.result()
...
# =====================================================================
# ENGINE (rest of the code – same as before)
# =====================================================================
class WhaleEngine:
    def __init__(self):
        self.dashboard = []
    ...
    def run_strategy10(self, symbol):
        ...
        for tf in ["5m", "15m"]:
            try:
                ...
                result = fetch_candles_ccxt(symbol, tf, limit)
                if result is None:
                    continue
    ...
    # ---- MAIN LOOP ----
    def run(self):
        log.info(f"WhaleTrader Pro (CCXT) – Scanning every {SCAN_INTERVAL//60} minutes.")
        while True:
            try:
                self.scan_market()
