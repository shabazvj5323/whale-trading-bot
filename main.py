...
def market_regime(closes, highs, lows):
    ...
    ema200 = ema_calc(closes, min(100, len(closes)-1))[-1]  # 100 more reliable with 1-day data
...
def darvas_box_strategy(opens, highs, lows, closes, vols, box_period=20, vol_period=20):
    ...
    # Step 2: Volume — CoinGecko OHLC has no real candle volume
    # Use price action breakout only (volume confirmation skipped)
...
class WhaleEngine:
    ...
    def score_signal(self, opens, highs, lows, closes, vols, closes15):
        ...
        if regime == "SIDEWAYS":
            adaptive_min = 5   # Sideways allowed — RSI/BB still valid
        elif atr_pct > 1.5:
            adaptive_min = 4   # High volatility = more opportunities
        elif atr_pct < 0.5:
            adaptive_min = 5   # Low volatility
        else:
            adaptive_min = 5   # Normal market
        ...
        # 4. Regime filter (+2) — bonus for trading WITH trend, no penalty
        if direction:
            if regime == "BULL" and direction == "buy":
                ...
                # No penalty — RSI/BB signals valid in sideways too
            else:
                reasons.append("COUNTER-TREND")
                # No penalty — let indicators decide
        ...
        # 6. Candle body strength (+2) — works with real OHLC from CoinGecko
        # Volume from CoinGecko is fake (all same), so use price action instead
