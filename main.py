# main.py – WhaleTrader Ultimate (GitHub 15-min, fixed data fetch)
import time
import logging
import os
import json
...
# =====================================================================
# DATA FETCHING – FIXED FOR GITHUB
# =====================================================================
            ...
            if data.get("code") == "0" and data.get("data"):
                ...
                if len(raw) > 0:
                    arr = np.array([[float(c[1]), float(c[2]), float(c[3]),
                                     float(c[4]), float(c[5])] for c in raw], dtype=float)
                    return arr[:,0], arr[:,1], arr[:,2], arr[:,3], arr[:,4]
        log.warning(f"OKX public {resp.status_code} for {symbol} {interval} – data empty or invalid")
        return None
