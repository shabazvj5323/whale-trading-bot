...
# Signal
MIN_SCORE   = 4        # Lower = more trade signals
VOL_MULT    = 1.1
BB_PERIOD   = 15
BB_STD      = 1.3
RSI_PERIOD  = 7
...
TP_MULT     = 2.0      # Initial TP (used as trailing activation zone)
SL_MULT     = 0.5      # SL distance
TRAIL_STEP  = 0.3      # Trail TP by this ATR multiplier each step

# Risk management
MAX_POSITIONS   = 6      # max open trades at once
...
class WhaleEngine:
    ...
    def score_signal(self, opens, highs, lows, closes, vols, closes15):
        ...
        # 4. Price momentum (+2) — recent candle direction confirms signal
    ...
    def check_positions(self, symbol, closes):
        ...
        # Include last known live price at start of closes array for immediate SL check
        last_price = float(self.state["last_prices"].get(symbol, closes[-1]))
        all_prices = np.append(np.array([last_price]), closes.astype(float))
