# main.py – WhaleTrader Ultimate (Full Updated with CVD + MIN_SCORE 4)
import time
import logging
import os
import json
...
# ---- Position Limit effectively removed ----
MAX_POSITIONS = 20

# ---- Indicators ----
MIN_SCORE = 4   # <-- CHANGED TO 4
VOL_MULT = 1.5
BB_PERIOD = 20
BB_STD = 2.0
BB_FAST_P = 10
...
def darvas_box_strategy(opens, highs, lows, closes, vols, box_period=10, vol_period=15):
...
def estimate_liquidation_zones(highs, lows, closes, atr, lookback=20, leverage=25):
    if len(highs) < lookback:
        return None, None, None, None
    swing_high = np.max(highs[-lookback:])
    swing_low = np.min(lows[-lookback:])
    current_price = closes[-1]
    liq_high = swing_high + atr * 0.5
    liq_low = swing_low - atr * 0.5
    near_high = abs(current_price - liq_high) / current_price < 0.005
    near_low = abs(current_price - liq_low) / current_price < 0.005
    if near_high:
        return "short_liquidation", liq_high, swing_high, atr
    if near_low:
        return "long_liquidation", liq_low, swing_low, atr
    return None, None, None, None

# =====================================================================
# DATA FETCHING (Enhanced with 1h)
# =====================================================================
...
def fetch_symbol_data_enhanced(symbol):
    """Fetch 1h, 15m, 5m, 1m data in parallel"""
...
class WhaleEngine:
    ...
    # ---- SCORING (UPDATED WITH CVD+ABSORPTION, MIN_SCORE=4, THRESHOLD=0.25) ----
    def score_signal(self, data):
        ...
        if regime in ("SIDEWAYS", "RANGING"):
            ...
            # ---- 5a. UPDATED ORDER FLOW (CVD + ABSORPTION) ----
            cvd = 0
            for i in range(-10, 0):
            ...
            # Absorption: high volume but delta near zero (institutional accumulation)
            ...
            if absorption:
                reasons.append("ABSORPTION (Hold)")  # No trade, just info
        ...
        # ---- 9. SMC: CHoCH + LIQUIDATION ----
        choch_dir, _ = detect_choch(highs5, lows5, closes5, lookback=10)
        if choch_dir == "bullish" and direction != "sell":
            score += 4; direction = direction or "buy"; reasons.append("CHoCH BULL")
        elif choch_dir == "bearish" and direction != "buy":
            score += 4; direction = direction or "sell"; reasons.append("CHoCH BEAR")

        liq_type, _, _, _ = estimate_liquidation_zones(highs5, lows5, closes5, atr, lookback=20)
        if liq_type == "short_liquidation" and direction != "sell":
            score += 3; direction = direction or "buy"; reasons.append("LIQ SHORTS")
        elif liq_type == "long_liquidation" and direction != "buy":
            score += 3; direction = direction or "sell"; reasons.append("LIQ LONGS")
    ...
    def strategy10_ema_crossover(self, symbol, opens, highs, lows, closes, vols, timeframe="5m"):
        try:
            if len(closes) < EMA10_SLOW + 2:
                return None, 0, []
            e10 = ema_calc(closes, EMA10_FAST)[-1]
            e20 = ema_calc(closes, EMA10_MID)[-1]
            e30 = ema_calc(closes, EMA10_SLOW)[-1]
            e10_prev = ema_calc(closes[:-1], EMA10_FAST)[-1]
            e20_prev = ema_calc(closes[:-1], EMA10_MID)[-1]
            e30_prev = ema_calc(closes[:-1], EMA10_SLOW)[-1]
            ...
            try:
                ...
                if direction and score >= 6 and self.can_trade(score):
                    ...
                    if direction == "buy":
                        ...
                        sl = round(min(swing_low - av * 0.2, price - av * SL_MULT), 6)
                    else:
                        tp = round(price - av * TP_MULT, 6)
                        swing_high = float(np.max(highs[-10:]))
                        sl = round(max(swing_high + av * 0.2, price + av * SL_MULT), 6)
    ...
    def scan_market(self):
        ...
        for symbol in SYMBOLS:
            try:
                ...
                score, direction, reasons, upper, lower, adaptive_min = self.score_signal(data)
...
"""
    ...
    # ---- MAIN LOOP ----
    def run(self):
        log.info(f"WhaleTrader Pro 24/7 Enhanced Mode — Scanning every {SCAN_INTERVAL//60} minutes. Press Ctrl+C to stop.")
        while True:
            try:
                self.scan_market()
