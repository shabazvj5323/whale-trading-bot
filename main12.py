...
# ═══════════════════════════════════════════════
#  CONFIG — SCALPING MODE (AGGRESSIVE)
# ═══════════════════════════════════════════════
...
MIN_SCORE   = 4          # FIXED: trade on score >= 4
VOL_MULT    = 1.3
BB_PERIOD   = 20
BB_STD      = 2.0
BB_FAST_P   = 10
...
TP_MULT     = 1.5        # Tighter for scalping
SL_MULT     = 0.5        # Tighter SL
TRAIL_LOOKBACK = 5      # Swing lookback for trailing
...
EMA10_SLOW      = 50
EMA10_VOL_MULT  = 5.0
EMA10_TIMEFRAMES = ["1m", "5m", "15m"]   # Added 1m for scalping
MAX_POSITIONS   = 10

# ── DAILY LOSS LIMIT REMOVED ────────────────
# MAX_DAILY_LOSS removed – unlimited trading
...
# ── PRICE ACTION & SMART MONEY (SMC) DETECTORS ──
...
def detect_pin_bar(highs, lows, closes):
    if len(highs) < 2:
        return None
    body = abs(closes[-1] - highs[-2])  # using previous close as reference? Actually pin bar: long wick.
    # Bullish pin: lower wick > 2*body
...
def detect_order_block(highs, lows, closes):
    # Simple: last consolidation range (last 5 bars) and breakout with volume
    if len(highs) < 6:
        return None
...
def detect_fair_value_gap(highs, lows, closes):
    # FVG: gap between previous candle's high/low and current opposite
    if len(highs) < 3:
        return None
    # Bullish FVG: gap up
    if lows[-1] > highs[-2]:
...
def detect_liquidity_sweep(highs, lows, closes):
    # Break of previous swing high/low with momentum
    if len(highs) < 10:
        return None
...
class WhaleEngine:
    ...
    # ── Risk check ── DAILY LIMIT REMOVED ──
    ...
    def score_signal(self, opens1, highs1, lows1, closes1, vols1,
        ...
        is_blast = False
        price  = float(closes1[-1])   # use 1m price for precision
        ...
        body_prev = float(closes1[-2]) - float(opens1[-2]) if len(opens1) >= 2 else 0
        ...
        # --- STRATEGIES DEPENDING ON REGIME (scalping) ---
        if regime in ("SIDEWAYS", "RANGING"):
            # Mean reversion, but we also add PA/SMC
            if not is_blast:
            ...
            # Trend following – add PA/SMC signals
            try:
                if len(closes5) >= 50:
        ...
        # ── PRICE ACTION & SMART MONEY (always apply) ──
        # Engulfing
        engulf = detect_engulfing(opens1, closes1)
        if engulf == "bullish":
            score += 3; direction = direction or "buy"; reasons.append("ENGULFING BULL")
        elif engulf == "bearish":
            score += 3; direction = direction or "sell"; reasons.append("ENGULFING BEAR")
        # Pin bar
        pin = detect_pin_bar(highs1, lows1, closes1)
        if pin == "bullish":
            score += 2; direction = direction or "buy"; reasons.append("PIN BAR BULL")
        elif pin == "bearish":
            score += 2; direction = direction or "sell"; reasons.append("PIN BAR BEAR")
        ...
        if ob == "bullish":
            score += 4; direction = direction or "buy"; reasons.append("OB BREAKOUT")
        elif ob == "bearish":
            score += 4; direction = direction or "sell"; reasons.append("OB BREAKDOWN")
        # FVG
        fvg = detect_fair_value_gap(highs1, lows1, closes1)
        if fvg == "bullish":
            score += 3; direction = direction or "buy"; reasons.append("FVG BUY")
        elif fvg == "bearish":
            score += 3; direction = direction or "sell"; reasons.append("FVG SELL")
        # Liquidity sweep
        sweep = detect_liquidity_sweep(highs1, lows1, closes1)
        if sweep == "bullish":
            score += 4; direction = direction or "buy"; reasons.append("LIQ SWEEP UP")
        elif sweep == "bearish":
            score += 4; direction = direction or "sell"; reasons.append("LIQ SWEEP DN")
        ...
        return score, direction, 0, upper, lower, reasons, adaptive_min

    # ── Position management with aggressive trailing ──
    def check_positions(self, symbol, current_price, closes_1m=None, highs_1m=None, lows_1m=None):
        ...
        # ── Trailing SL using swing highs/lows (1m data) ──
        if closes_1m is not None and len(closes_1m) >= TRAIL_LOOKBACK:
            if side == "buy":
                # trail SL to lowest low of last N bars minus buffer
    ...
    # ── Main run (now in infinite loop) ──
    def run(self):
        log.info("WhaleTrader Pro — SCALPING MODE ACTIVE (every 60s)")
        while True:
            self.dashboard = []   # reset each cycle
            ...
            for symbol in symbols_this_run:
                try:
                    ...
                    if result is None:
                        ...
                        self.dashboard.append({"symbol": symbol, "price": last_price,
                                               "signal": "SCANNING", "score": 0,
                                               "entry": None, "tp": None, "sl": None,
                                               "reasons": ["Data unavailable"]})
                        continue
                    opens1, highs1, lows1, closes1, vols1, opens5, highs5, lows5, closes5, vols5, closes15 = result

                    current_price = float(closes1[-1])  # use 1m close
                    ...
                    if direction and score >= adaptive_min and self.can_trade(score):
                        ...
                        if direction == "buy":
                            tp = round(price + av * TP_MULT, 6)
                            swing_low = float(np.min(lows1[-10:]))  # 1m swing
            ...
            log.info("Cycle complete. Sleeping 60s...")
            time.sleep(60)   # run every minute

    # ── Dashboard (unchanged) ──
    def build_dashboard(self):
        # (Keep exactly the same as original to preserve UI)
        state  = self.state
...
if __name__ == "__main__":
    engine = WhaleEngine()
    engine.run()   # infinite loop inside