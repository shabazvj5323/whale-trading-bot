...
# TP/SL
TP_MULT     = 2.5      # Wider TP — more profit per trade
SL_MULT     = 0.7      # Tighter SL — better R:R ratio 3.5:1
TRAIL_STEP  = 0.15

# Risk management
MAX_DAILY_LOSS  = 80.0
MARGIN_BLAST    = 100.0  # BB Blast margin — same as normal, safety first

# Hedge Config — ONLY on BB Blast squeeze
HEDGE_ENABLED   = True   # Activate hedge on BB Blast
HEDGE_SUFFIX    = "_HEDGE"  # Hedge position key suffix

# ── Strategy 10: EMA Fresh Crossover + Volume Spike ──
EMA10_FAST      = 20     # EMA 20
EMA10_MID       = 30     # EMA 30
EMA10_SLOW      = 50     # EMA 50
EMA10_VOL_MULT  = 5.0    # Volume spike multiplier (configurable)
EMA10_TIMEFRAMES = ["5m", "15m", "30m"]  # All 3 timeframes run independently
MAX_POSITIONS   = 10     # 6 normal + 2 hedge + 2 strategy10 slots
...
class WhaleEngine:
    ...
    def can_trade(self, score=0):
        """Only daily loss limit applies — no position limit."""
    ...
    def score_signal(self, opens, highs, lows, closes, vols, closes15):
        ...
        # BULL  → Trend strategies (EMA Ribbon, S/R Breakout, VWAP)
        # BEAR  → Trend strategies (reversed: sell side)
        # SIDEWAYS/RANGING → Mean reversion (BB Bounce, Double BB)
        # BB BLAST → Always active (direction-agnostic)
        # ══════════════════════════════════════════════════════

        # ════════════════════════════════════════
        # STRATEGY 3: BB BLAST — Always active regardless of regime
        # ════════════════════════════════════════
        band_widths = []
        for i in range(-20, -1):
            try:
        ...
        if regime in ("SIDEWAYS", "RANGING"):
            # SIDEWAYS: ONLY BB Blast allowed (already scored above)
            # No mean reversion — choppy market = false signals
            # Direction must come from BB Blast only
            if not is_blast:
                # No blast = no trade in sideways
                score = 0
                direction = None
                reasons.append("SIDEWAYS-SKIP")
            ...
            # Order Flow also good in sideways
            try:
            ...
            # ════════════════════════════════════════
            # BULL: Trend-following strategies — BUY side preferred
            # EMA Ribbon + VWAP + S/R Breakout
            # ════════════════════════════════════════
            try:
                if len(closes) >= 50:
                    e9 = ema_calc(closes,9)[-1]; e21=ema_calc(closes,21)[-1]; e50=ema_calc(closes,50)[-1]
                    if e9 > e21 > e50:
            ...
            try:
                ...
                if price < vwap * 0.999:
                    ...
                    # BULL mein SELL = counter-trend = BLOCK
                    if direction == "sell":
                        direction = None
                        score = max(0, score - 4)
            except: pass
            ...
            # BB Bounce also valid in Bull — buy dips
            if price <= lower:
            ...
            # ════════════════════════════════════════
            # BEAR: Trend-following — SELL side preferred
            # EMA Ribbon + VWAP + S/R Breakout reversed
            # ════════════════════════════════════════
            try:
                if len(closes) >= 50:
                    e9 = ema_calc(closes,9)[-1]; e21=ema_calc(closes,21)[-1]; e50=ema_calc(closes,50)[-1]
            ...
            try:
                ...
                if price > vwap * 1.001:
                    ...
                    # BEAR mein BUY = counter-trend = BLOCK
                    if direction == "buy":
                        direction = None
                        score = max(0, score - 4)
            except: pass
            ...
            # BB Bounce also valid in Bear — sell rallies
            if price >= upper:
    ...
    def check_positions(self, symbol, current_price):
        ...
        if hit:
            ...
            # Extract strategy name from reasons for tracking
    ...
    # ══════════════════════════════════════════════════════
    # STRATEGY 10: EMA FRESH CROSSOVER + VOLUME SPIKE
    # Completely isolated — runs independently on 3 timeframes
    # Does NOT modify existing strategies 1-9
    # ══════════════════════════════════════════════════════
    def strategy10_ema_crossover(self, symbol, opens, highs, lows, closes, vols, timeframe="5m"):
        """
        EMA Fresh Crossover + Volume Spike Strategy
        BUY:  Fresh bullish crossover (EMA20>EMA30>EMA50 just happened) + 
              green candle + volume spike (current > prev * EMA10_VOL_MULT)
        SELL: Fresh bearish crossover (EMA20<EMA30<EMA50 just happened) + 
              red candle + volume spike
        ...
        try:
            ...
            # ── Calculate EMAs current and previous candle ──
            ...
            # ── Fresh crossover = alignment just happened ──
            ...
            # ── Volume spike: current > previous * multiplier ──
            vol_now  = float(vols[-1])
            ...
            if fresh_bull and green_candle and vol_spike:
                reasons = [
                    f"S10 FRESH BULL CROSS [{timeframe}]",
                    f"EMA20>{round(e20_now,4)} EMA30>{round(e30_now,4)} EMA50>{round(e50_now,4)}",
                    f"VOL {round(vol_now/vol_prev,1)}x SPIKE",
                    f"GREEN CANDLE"
                ]
                log.info(f"S10 BUY SIGNAL: {symbol} [{timeframe}] | Price:${price} | Vol:{round(vol_now/vol_prev,1)}x | E20:{round(e20_now,4)}")
                return "buy", 8, reasons

            if fresh_bear and red_candle and vol_spike:
                reasons = [
                    f"S10 FRESH BEAR CROSS [{timeframe}]",
                    f"EMA20<{round(e20_now,4)} EMA30<{round(e30_now,4)} EMA50<{round(e50_now,4)}",
                    f"VOL {round(vol_now/vol_prev,1)}x SPIKE",
                    f"RED CANDLE"
                ]
                log.info(f"S10 SELL SIGNAL: {symbol} [{timeframe}] | Price:${price} | Vol:{round(vol_now/vol_prev,1)}x | E20:{round(e20_now,4)}")
    ...
    def run_strategy10(self, symbol):
        """
        Run Strategy 10 on all 3 timeframes independently.
        Completely isolated from strategies 1-9.
        Uses existing risk management framework.
        """
        s10_key = symbol + "_S10"

        # Skip if already in a S10 position for this symbol
        ...
        # Run on all 3 timeframes independently
        for tf in EMA10_TIMEFRAMES:
            try:
                limit = 120 if tf == "5m" else 80 if tf == "15m" else 60
                ...
                if direction and score >= 6 and self.can_trade(score):
                    ...
                    if direction == "buy":
                        ...
                        # SL = swing low of last 10 candles (PA based)
                        swing_low = float(np.min(lows[-10:]))
                        sl = round(min(swing_low - av * 0.1, price - av * SL_MULT), 6)
                    else:
                        tp = round(price - av * TP_MULT, 6)
                        # SL = swing high of last 10 candles (PA based)
    ...
    def run(self):
        ...
        self.check_hedge_positions()  # Check hedge positions each cycle

        # ── Strategy 10: Run independently for each symbol ──
        ...
        for symbol in symbols_this_run:
            try:
                ...
                if direction and score >= adaptive_min and self.can_trade(score):
                    ...
                    # ── HEDGE — Only on BB Blast Squeeze ──────────────
                    # Conditions: BB Blast detected + score >= 6 + hedge slot free
                    ...
                    if (HEDGE_ENABLED and is_blast and score >= 6 
                        ...
                        # Hedge TP/SL = opposite side
                        hedge_margin = MARGIN_BLAST  # same blast margin
                        if hedge_direction == "buy":
                            hedge_tp = round(price + av * TP_MULT, 6)
                            swing_low = float(np.min(lows[-10:]))
                            hedge_sl = round(min(swing_low - av * 0.5, price - av * SL_MULT), 6)
                        else:
                            hedge_tp = round(price - av * TP_MULT, 6)
                            swing_high = float(np.max(highs[-10:]))
                            hedge_sl = round(max(swing_high + av * 0.5, price + av * SL_MULT), 6)
                ...
                # Show Strategy 10 position in dashboard if exists
                s10_key = symbol + "_S10"
                if s10_key in self.state["active_positions"]:
                ...
                # Show hedge position in dashboard if exists
                hedge_key = symbol + HEDGE_SUFFIX
    ...
    def build_dashboard(self):
        ...
        trading_ok  = daily_loss < MAX_DAILY_LOSS
...
"""
        ...
        for t in trades:
            ...
            if matched in strat_stats:
                ...
                if pnl_t > 0: strat_stats[matched]["wins"]   += 1
                else:         strat_stats[matched]["losses"] += 1
