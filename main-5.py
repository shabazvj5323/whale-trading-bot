...
SYMBOLS          = ["BTC/USDT", "ETH/USDT", "SOL/USDT", "BNB/USDT", "XRP/USDT", "DOGE/USDT",
                   "AVAX/USDT", "LINK/USDT", "ADA/USDT", "DOT/USDT", "LTC/USDT", "MATIC/USDT"]
...
class WhaleEngine:
    ...
    def fetch_coingecko_ohlc(self, symbol, limit=120):
        ...
        try:
            ...
            lows   = arr[:, 3]
            closes = arr[:, 4]
            vols   = self.fetch_coingecko_volume(coin_id, len(closes))
            closes15 = closes[::3]
            n = min(limit, len(closes))
            return opens[-n:], highs[-n:], lows[-n:], closes[-n:], vols[-n:], closes15
        except Exception as e:
            log.warning(f"CoinGecko OHLC failed {symbol}: {e}")
            return None

    def fetch_coingecko_volume(self, coin_id, length):
        try:
            ...
            r.raise_for_status()
            data = r.json()
            vols_raw = [v[1] for v in data.get("total_volumes", [])]
            if len(vols_raw) < 2:
                return np.ones(length) * 1000
            arr = np.array(vols_raw, dtype=float)
            arr = arr / (arr.mean() + 1e-9) * 1000
            if len(arr) < length:
    ...
    def fetch_data(self, symbol, limit=120):
        ...
        result = self.fetch_coingecko_ohlc(symbol, limit)
