...
class WhaleEngine:
    ...
    def score_signal(self, opens, highs, lows, closes, vols, closes15):
        ...
        if regime in ("SIDEWAYS", "RANGING"):
            ...
            # BB Bounce + Double BB Zone — best in ranging
            # ════════════════════════════════════════
            if price <= lower:
                score += 4
            ...
            try:
                ...
                if price < vwap * 0.999:
                    score += 3; direction = direction or "buy"; reasons.append("VWAP BUY")
                elif price > vwap * 1.003:
                    score += 2; direction = direction or "sell"; reasons.append("VWAP SELL")
            except: pass
            ...
            try:
                ...
                vwap = float(np.sum(typical * vols) / (np.sum(vols) + 1e-9))
                if price > vwap * 1.001:
                    score += 3; direction = direction or "sell"; reasons.append("VWAP SELL")
                elif price < vwap * 0.997:
                    score += 2; direction = direction or "buy"; reasons.append("VWAP BUY")
            except: pass
