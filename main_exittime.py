...
class WhaleEngine:
    ...
    # ── Position management ──────────────────
    def check_positions(self, symbol, closes, timestamps=None):
        ...
        for i, price in enumerate(closes.astype(float)):
            ...
            if hit:
                ...
                # Get actual exit time from candle timestamp
                if timestamps is not None and i < len(timestamps):
                    exit_ts = int(timestamps[i]) / 1000  # ms to seconds
                    from datetime import timezone
                    exit_dt = datetime.fromtimestamp(exit_ts, tz=timezone.utc) + timedelta(hours=5, minutes=30)
                    exit_time_str = exit_dt.strftime("%m-%d %H:%M")
                else:
        ...
        for symbol in symbols_this_run:
            try:
                ...
                if result is None:
                    ...
                    continue
                opens, highs, lows, closes, vols, closes15, timestamps = result

                # Check existing position first
                self.check_positions(symbol, closes, timestamps)
