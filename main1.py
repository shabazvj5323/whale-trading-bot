# main.py – Improved Scalping with Trailing SL + Dashboard Update
import time
import logging
import os
import json
...
class WhaleEngine:
    ...
    # ---- DATA FETCH ----
    def fetch_binance_public(self, symbol, interval="5m", limit=120):
        try:
    ...
    def fetch_candles(self, symbol, interval="5m", limit=120):
        result = self.fetch_binance_public(symbol, interval, limit)
        if result is not None:
            return result
        return self.fetch_okx_public(symbol, interval, limit)

    def fetch_symbol_data(self, symbol):
        with ThreadPoolExecutor(max_workers=3) as executor:
            future_1m = executor.submit(self.fetch_candles, symbol, "1m", 60)
            future_5m = executor.submit(self.fetch_candles, symbol, "5m", 120)
            future_15m = executor.submit(self.fetch_candles, symbol, "15m", 60)
        ...
        if regime in ("SIDEWAYS", "RANGING"):
            ...
            if not is_blast:
                if l_fast <= price <= lower and rsi < 30:
                    score += 5
                    direction = "buy"
                    reasons.append("DOUBLE BB + RSI<30")
                elif upper <= price <= u_fast and rsi > 70:
                    score += 5
                    direction = "sell"
                    reasons.append("DOUBLE BB + RSI>70")
            try:
            ...
            if len(closes5) >= 30:
                e9 = ema_calc(closes5, 9)[-1]
                e21 = ema_calc(closes5, 21)[-1]
                e50 = ema_calc(closes5, 50)[-1]
                if e9 > e21 > e50:
                    score += 5
                    direction = direction or "buy"
            ...
            try:
                ...
                if price > vwap * 1.002 and price < vwap * 1.005:
                    score += 3
                    direction = "sell"
                    reasons.append("VWAP RALLY")
            except:
                pass
            if len(highs5) >= SR_PERIOD:
                resistance = float(np.max(highs5[-SR_PERIOD:]))
                support = float(np.min(lows5[-SR_PERIOD:]))
                if price < support * (1 - SR_THRESH) and vol_spike:
                    score += 5
                    direction = direction or "sell"
                    reasons.append("SR BREAKOUT DN")
    ...
    def run_strategy10(self, symbol):
        ...
        if s10_key in self.state["active_positions"]:
            ...
            if price:
                ...
                if hit:
                    ...
                    if pnl > 0:
                        st["wins"] += 1
                        st["best_trade"] = round(max(st.get("best_trade", 0), pnl), 4)
                    else:
                        st["losses"] += 1
                        st["worst_trade"] = round(min(st.get("worst_trade", 0), pnl), 4)
                    today = ist_now().strftime("%Y-%m-%d")
                    st["daily_pnl"][today] = round(st["daily_pnl"].get(today, 0) + pnl, 4)
                    st["daily_trades"][today] = st["daily_trades"].get(today, 0) + 1
                    self.state["trades"].append({
        ...
        for tf in EMA10_TIMEFRAMES:
            try:
                limit = 120 if tf == "3m" else 80 if tf == "5m" else 60
                result = self.fetch_candles(symbol, tf, limit)
                if result is None:
                    continue
...
"""
    ...
    # ---- MAIN LOOP ----
    def run(self):
        log.info(f"WhaleTrader Pro 24/7 Improved Scalping Mode — Scanning every {SCAN_INTERVAL//60} minutes. Press Ctrl+C to stop.")
        while True:
            try:
                self.scan_market()
